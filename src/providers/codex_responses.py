...
Pure request/response shaping + a stateful streaming decoder, mirroring the two
PR1 transports. Speaks the codex Responses API behind a ChatGPT Plus/Pro OAuth
session (`POST {base}/responses`, `store:false`, low-level streamed events) rather
than the metered chat/completions API.

Authoritative wire crib (validated against pi + Hermes, both running this exact
pattern on remote-SSH deploys):
  - request body  : pi `openai-codex-responses.ts` `buildRequestBody`
  - SSE events    : pi `openai-responses-shared.ts` `processResponsesStream`
  - golden stream : pi `test/openai-codex-stream.test.ts`
  - headers       : `hermes-openai-codex-implementation-spec.md` §8

Boundaries (kept deliberately, per PR2 plan):
...
# reuse). Tracked against upstream; see the Hermes spec §8.
    ...
    for m in messages:
        ...
        if role == "system":
            ...
            for tc in m.get("tool_calls") or []:
                ...
                # reasoning-item pairing OpenAI runs on stored sessions, which PR2
                # does not use (store:false, no reasoning echo). call_id pairs the
                # result below.
                input_items.append({
