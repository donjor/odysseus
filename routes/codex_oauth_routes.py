...
def _purge_endpoint_rows(db, endpoint_id: str) -> None:
    """Delete an endpoint's token row and detach its attempt log rows. Done in
    code, not via DB ON DELETE — this app does not enable SQLite FK enforcement,
    so the schema's CASCADE/SET NULL are documentation, not runtime behavior.
    Caller commits."""
...
async def _run_login_poll(attempt_id: str) -> None:
    ...
    if data is None or not data.device_auth_id:
        return
    deadline = data.expires_at or (_utcnow_naive() + cx._timedelta(cx.DEFAULT_DEVICE_CODE_TTL_SECONDS))
