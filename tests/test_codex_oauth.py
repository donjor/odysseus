"""PR2a tests: the ChatGPT-subscription (codex) OAuth backend.
...
status/cancel/disconnect/import routes. All network I/O is respx-mocked; the DB
is a throwaway sqlite rebind. No real tokens, no real OpenAI calls.
"""
import asyncio
import base64
import json
...
class TestCodexOAuthService:
    @respx.mock
    async def test_device_code_request(self):
        respx.post(cx.DEVICE_USERCODE_URL).mock(return_value=httpx.Response(200, json={
    ...
    @respx.mock
    async def test_device_code_interval_clamped(self):
        respx.post(cx.DEVICE_USERCODE_URL).mock(return_value=httpx.Response(200, json={
    ...
    @respx.mock
    async def test_poll_pending_then_success(self):
        route = respx.post(cx.DEVICE_TOKEN_URL)
        route.side_effect = [
    ...
    @respx.mock
    async def test_exchange_and_account_id(self):
        access = _jwt()
        respx.post(cx.TOKEN_URL).mock(return_value=httpx.Response(200, json={
    ...
    @respx.mock
    async def test_refresh_rotates_when_new_token_returned(self):
        respx.post(cx.TOKEN_URL).mock(return_value=httpx.Response(200, json={
    ...
    @respx.mock
    async def test_refresh_keeps_prior_when_no_new_token(self):
        respx.post(cx.TOKEN_URL).mock(return_value=httpx.Response(200, json={"access_token": _jwt()}))
        ts = await cx.refresh_tokens("r-keep")
        assert ts.refresh_token == "r-keep"

    @respx.mock
    async def test_refresh_429_is_quota_no_relogin(self):
        respx.post(cx.TOKEN_URL).mock(return_value=httpx.Response(429, json={}))
    ...
    @respx.mock
    async def test_refresh_invalid_grant_requires_relogin(self):
        respx.post(cx.TOKEN_URL).mock(return_value=httpx.Response(400, json={"error": "invalid_grant"}))
        with pytest.raises(cx.CodexAuthError) as ei:
            await cx.refresh_tokens("r")
...
class TestResolveOAuth:
    ...
    async def test_refresh_on_expiry_persists_rotation(self, oauth_db):
    ...
    async def test_single_flight_refresh(self, oauth_db):
        _seed_token(oauth_db, "ep-sf", _jwt(exp_offset=10), "r", _utcnow() - _delta(5))
        with respx.mock:
            route = respx.post(cx.TOKEN_URL).mock(return_value=httpx.Response(200, json={
        ...
        assert route.call_count == 1                     # 5 concurrent resolves -> 1 refresh
    ...
    async def test_quota_maps_to_429_and_persists_status(self, oauth_db):
    ...
    async def test_invalid_grant_maps_to_401_and_needs_relogin(self, oauth_db):
...
class TestConnectRoutes:
    def test_connect_creates_pending_without_leaking_secrets(self, oauth_db, monkeypatch):
    ...
    def test_poll_loop_authorizes_and_enables(self, oauth_db, monkeypatch):
        monkeypatch.setattr(routes_mod, "_spawn", lambda coro: coro.close())
        access = _jwt()
        with respx.mock:
            respx.post(cx.DEVICE_USERCODE_URL).mock(return_value=httpx.Response(200, json={
            ...
            pr = respx.post(cx.DEVICE_TOKEN_URL)
            pr.side_effect = [httpx.Response(403, json={}),
                              httpx.Response(200, json={"authorization_code": "ac", "code_verifier": "cv"})]
            respx.post(cx.TOKEN_URL).mock(return_value=httpx.Response(200, json={
    ...
    def test_cancel_deletes_endpoint_but_keeps_attempt(self, oauth_db, monkeypatch):
        monkeypatch.setattr(routes_mod, "_spawn", lambda coro: coro.close())
        with respx.mock:
            respx.post(cx.DEVICE_USERCODE_URL).mock(return_value=httpx.Response(200, json={
                "user_code": "U", "device_auth_id": "d", "interval": 5, "expires_in": 900}))
    ...
    def test_owner_isolation_404(self, oauth_db, monkeypatch):
        monkeypatch.setattr(routes_mod, "_spawn", lambda coro: coro.close())
        with respx.mock:
            respx.post(cx.DEVICE_USERCODE_URL).mock(return_value=httpx.Response(200, json={
                "user_code": "U", "device_auth_id": "d", "interval": 5, "expires_in": 900}))
    ...
    def test_import_cli_fast_path(self, oauth_db, monkeypatch, tmp_path):
        access = _jwt()
        codex_dir = tmp_path / "home" / ".codex"
        codex_dir.mkdir(parents=True)
        (codex_dir / "auth.json").write_text(json.dumps({"tokens": {
            "access_token": access, "refresh_token": "r-cli", "account_id": "acct-xyz"}}))
        monkeypatch.setenv("HOME", str(tmp_path / "home"))
        body = _client().post("/api/providers/openai-codex/import-cli").json()
        assert body["status"] == "active"
        s = oauth_db()
        try:
            tok = s.query(db.EndpointOAuthToken).filter_by(endpoint_id=body["endpoint_id"]).first()
            assert tok.access_token == access and tok.refresh_token == "r-cli"
            assert s.get(db.ModelEndpoint, body["endpoint_id"]).is_enabled is True
        finally:
            s.close()
