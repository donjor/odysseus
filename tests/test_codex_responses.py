"""PR2b tests: the codex_responses transport, stream decoder, registry/spec wiring,
and the one additive engine guard (OAuth response-cache bypass).
...
so it is immune to the sys.modules stubbing other test files install) with respx
and a monkeypatched `auth.resolve` — no DB, no real OpenAI calls.
...
# ══════════════════════════════════════════════════════════════════════════════
# End-to-end through llm_core (respx-mocked codex backend)
# ══════════════════════════════════════════════════════════════════════════════
...
class TestEndToEndStream:
    async def test_stream_text_through_llm_core(self, monkeypatch):
        _patch_oauth_resolve(monkeypatch)
        sse = _sse(_ev_msg_added(), _ev_part_added(), _ev_text_delta("Hello"),
                   _ev_msg_done("Hello"), _ev_completed())
        with respx.mock:
            route = respx.post(CODEX_URL).mock(return_value=httpx.Response(200, content=sse))
    ...
    async def test_stream_tool_call_through_llm_core(self, monkeypatch):
        _patch_oauth_resolve(monkeypatch)
        sse = _sse(
            {"type": "response.output_item.added",
        ...
        with respx.mock:
            respx.post(CODEX_URL).mock(return_value=httpx.Response(200, content=sse))
            chunks = await _collect(llm_core.stream_llm(CODEX_URL, "gpt-5.4",
...
class TestOAuthCacheBypass:
    ...
    async def test_oauth_call_bypasses_cache(self, monkeypatch):
        _patch_oauth_resolve(monkeypatch)
        ref = _codex_ref(endpoint_id="ep-alice", owner="alice")
        with respx.mock:
            route = respx.post(CODEX_URL).mock(side_effect=[
    ...
    async def test_non_oauth_call_still_cached(self):
        # Control: api_key providers keep using the cache (guard is scoped to oauth).
        with respx.mock:
            route = respx.post(OPENAI_URL).mock(return_value=httpx.Response(
    ...
    def test_codex_excluded_from_fallback_candidates(self, monkeypatch):
        ...
        try:
            ...
            monkeypatch.setattr(er, "resolve_endpoint_by_id",
                                lambda eid, model: {"codex": codex, "oai": openai}.get(eid))
